...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        # 前面就是简单的经过了12层的bertencoder模块，没有加其他东西
        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)
...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # 获取了tokenembedding，typeembedding，positionembedding和hopembedding，四个embedding相加，LN和dropout后输出
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings


class GraphAttenTransformer(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        # call initialization method of its super class
        self.init_weights()   
    ...
    def forward(
        ...
        attention_masks: (batch_size, sequence_length)
            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        token_type_ids: (batch_size, sequence_length)
        position_ids: (batch_size, sequence_length)
        hops_ids: (batch_size, sequence_length)
        """
