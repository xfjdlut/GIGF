...
def calc_f1(hyps, refs):
    ...
    for response, golden_response in zip(hyps, refs):
        golden_response = "".join(golden_response)#把字符转换成正常的句子
...
def load_data(fp, is_gold=False, lower_case=True):
    ...
    with open(fp, 'r', encoding='utf-8') as fr:
        for line in fr:
            ...
            samples.append(resp)#把句子转换成token
            if is_gold:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_file", type=str, default="../outputs/output_test.txt", help="path of the generated response")
    parser.add_argument("--gold_file", type=str, default="../data/DuRecDial/new_sample_test.json", help="Path of the ground truth response")
