...
from reponseGeneration.model import ResGen
...
from dataset_gpt2 import GPT2Dataset
...
def sample_sequence(model, context, tokenizer, args):
    ...
    for i in range(args.max_dec_len):
        ...
        if args.top_k > 0 or (args.top_p > 0 and args.top_p <= 1):
            ...
            next_token = torch.topk(probs, 1)[1]#返回的是最大值的索引

        # if i < args.min_dec_len and next_token.item() in special_tokens_ids:
        #      next_token_all = torch.multinomial(probs, num_samples=4, replacement=False)
        #      for next_token_one in next_token_all:
        #          if next_token_one not in special_tokens_ids: #取出next_token其中的值
        #              next_token=next_token_one.unsqueeze(0)
        #              break
        if i < args.min_dec_len and next_token.item() in special_tokens_ids:
...
def main():
    ...
    parser.add_argument('--use_goal', type=str2bool, default=True, help="Whether or not use goal-enhanced generation")
    parser.add_argument('--goal_path', type=str, default="../outputs/pred_goal.json", help="Path of the generated goal by goal planning module")
    parser.add_argument("--test_path", type=str, default="../data/DuRecDial/new_sample_test.json",
                        help="Path of the test dataset.")
    parser.add_argument("--type_dic_path", type=str, default="../data/DuRecDial/type_dict.json", help="type dict")
    parser.add_argument("--entity_dic_path", type=str, default="../data/DuRecDial/type_dict.json", help="entity dict")
    parser.add_argument("--cache_dir", type=str, default="../caches/responseGeneration",
                        help="Path or url of the dataset cache dir.")
    parser.add_argument("--model_dir", type=str, default="../logs/ResponseGeneration", help="Path, url or short name of the model")
    parser.add_argument("--output_dir", type=str, default="../outputs", help="Dir for storing generated output")
    ...
    model_dir = args.model_dir + '_w_Goal' if args.use_goal else args.model_dir
    ...
    test_data = load_data(tokenizer, logger, args.test_path, args.cache_dir,
                          data_partition="test", use_goal=args.use_goal, goal_path=args.goal_path,
                          type_dic=args.type_dic_path, entity_dic=args.entity_dic_path)

    logger.info("Evaluating...")
    eval_dataset = GPT2Dataset(test_data, tokenizer, max_seq_len=args.max_seq_len, lm_labels=True)
    ...
    with torch.no_grad():
        for batch in tqdm(eval_loader):
            ...
            lm_output = model(input_ids, lm_labels)
            lm_loss = lm_output[0]
    ...
    # set output dir
    output_dir = args.output_dir + '_w_Goal' if args.use_goal else args.output_dir
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_path = os.path.join(output_dir, "output_test.txt")

    logger.info("Generating...")
    test_dataset = GPT2Dataset(test_data, tokenizer, max_seq_len=args.max_seq_len, lm_labels=False)
